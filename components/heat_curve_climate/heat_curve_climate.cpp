...
float HeatingCurveClimate::get_heat_curve_temp() {
  ...
  ESP_LOGD(TAG, "Ecart T: %.1fK", this->delta_);
  ESP_LOGD(TAG, "Outdoor temperature: %.1f°C", this->outdoor_temp_);
  ESP_LOGD(TAG, "Indoor temperature: %.1f°C", this->current_temperature);
